 ...
 * @code
 * const FusionQuaternion quaternion = FUSION_QUATERNION_IDENTITY;
 * const FusionEulerAngles eulerAngles = FusionQuaternionToEulerAngles_deg(quaternion);
 * @endcode
 *
 * @param quaternion Quaternion to be converted.
 * @return Euler angles in degrees.
 */
static inline __attribute__((always_inline)) FusionEulerAngles FusionQuaternionToEulerAngles_deg(const FusionQuaternion quaternion) {
#define Q quaternion.element // define shorthand label for more readable code
 ...
 * @code
 * const FusionQuaternion quaternion = FUSION_QUATERNION_IDENTITY;
 * const FusionEulerAngles eulerAngles = FusionQuaternionToEulerAngles_rad(quaternion);
 * @endcode
 *
 * @param quaternion Quaternion to be converted.
 * @return Euler angles in radians.
 */
static inline __attribute__((always_inline)) FusionEulerAngles FusionQuaternionToEulerAngles_rad(const FusionQuaternion quaternion) {
#define Q quaternion.element // define shorthand label for more readable code
    const float qwSquaredMinusHalf = Q.w * Q.w - 0.5f; // calculate common terms to avoid repeated operations
    const FusionEulerAngles eulerAngles = {
        .angle.roll = atan2f(Q.y * Q.z - Q.w * Q.x, qwSquaredMinusHalf + Q.z * Q.z),
        .angle.pitch = -asinf(2.0f * (Q.x * Q.z + Q.w * Q.y)),
        .angle.yaw = atan2f(Q.x * Q.y - Q.w * Q.z, qwSquaredMinusHalf + Q.x * Q.x),
    };
    return eulerAngles;
#undef Q // undefine shorthand label
}
